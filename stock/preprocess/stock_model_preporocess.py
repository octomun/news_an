...
for fetch in range(len(fetch)):
    try :
        stock_setting.drop_table_model_pro(conn, code.values[fetch])
        stock_setting.create_table_model_pro(conn, code.values[fetch])
    except:
        stock_setting.create_table_model_pro(conn, code.values[fetch])
    stock = stock_setting.fetch_all(conn,code.values[fetch])
    ...
    for i in range(len(stock)):
        stock_setting.insert_tabel_model_pro(dates=stock['dates'][i],times=stock['times'][i],opens= float(stock['opens'][i]),
    ...
    stock_setting.drop_table(conn,code.values[fetch])