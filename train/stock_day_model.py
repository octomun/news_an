...
import os
os.chdir('F:/stock')
stock= pd.read_csv('stock_dropna_sort_day_2000.csv', encoding='utf-8')
standard=1
...
for i in range(len(train_data)-days):
    ...
    #     continue
    x_train.append(stock[i:days+i][['pct','5days_rolling','10days_rolling','max_min','week_day','month']])
...
for i in range(len(test_data)):
    ...
    #     continue
    x_test.append(stock[-testing_data_len+i-days:-testing_data_len+i][['pct','5days_rolling','10days_rolling','max_min','week_day','month']])
...
with open(f'x_train.json', "w") as file_write:
    json.dump(x_train, file_write)
with open(f'y_train.json', "w") as file_write:
    json.dump(y_train, file_write)
with open(f'x_test.json', "w") as file_write:
    json.dump(x_test, file_write)
...
model = Sequential()
model.add(LSTM(x_train.shape[1], return_sequences=True, input_shape= (x_train.shape[1], 6)))
...
mc = ModelCheckpoint('best_model_day.h5', monitor='acc', mode='max', verbose=1, save_best_only=True)# Train the model
...
# train = stock.iloc[:testing_data_len]
train = stock[:-testing_data_len]
train.to_csv('train123123.csv')
# valid = stock.iloc[-testing_data_len:]
...
# a['closes']=a['closes'][:-testing_data_len]
